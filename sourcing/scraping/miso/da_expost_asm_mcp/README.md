...
Production-ready HTTP/REST API scraper for collecting MISO Day-Ahead Ex-Post Ancillary Services Market Clearing Prices (ASM MCP) data.
...
This scraper collects ancillary services market clearing prices from MISO's Day-Ahead Ex-Post market. The data includes final, settled Market Clearing Prices (MCP) for six ancillary service products across eight reserve zones. Ex-Post prices represent the actual cleared prices after the market has finalized, making them essential for financial settlement and performance analysis.
...
```
https://apim.misoenergy.org/pricing/v1/day-ahead/{YYYY-MM-DD}/asm-expost
```
...
```
s3://{bucket}/sourcing/miso_da_expost_asm_mcp/
  year=2024/
    month=12/
      day=01/
        da_expost_asm_mcp_20241201.json.gz
```
...
- **Naming:** `da_expost_asm_mcp_YYYYMMDD.json.gz`
...
```bash
pytest tests/ -v \
  --cov=sourcing.scraping.miso.da_expost_asm_mcp \
  --cov-report=html
```
...
### Ex-Post (This Scraper)
- **Final settled prices** after market clearing
- Used for financial settlement and reconciliation
- Published at 2:00 PM EST day before operating day
- Fixed prices - do not change after publication
- Essential for billing and settlement processes

### Ex-Ante (See `da_exante_asm_mcp` scraper)
- **Forecast/expected prices** before market clearing
- Used for planning and forecasting
- Available earlier than ex-post
- Subject to change until market clears
- Essential for market strategy and planning
...
4. **Schedule regular collection:**
   ...
   0 15 * * * /path/to/venv/bin/python /path/to/scraper_miso_da_expost_asm_mcp.py \
     --start-date $(date +%Y-%m-%d) \
     --end-date $(date +%Y-%m-%d) \
     --environment prod
   ```
...
- See specification: `/tmp/miso-pricing-specs/spec-day-ahead-expost-mcp.md`
- Compare with Ex-Ante scraper: `../da_exante_asm_mcp/README.md`
