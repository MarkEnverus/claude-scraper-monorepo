"""Tests for MISO Wind Forecast Scraper.
...
def collector(mock_redis):
    ...
    return MisoWindForecastCollector(
...
def sample_forecast_data():
    """Load sample forecast data from fixtures."""
    fixture_path = Path(__file__).parent / "fixtures" / "sample_wind_forecast.json"
    with open(fixture_path, "r") as f:
        return json.load(f)


@pytest.fixture
def sample_forecast_bytes(sample_forecast_data):
    """Sample forecast data as bytes."""
    return json.dumps(sample_forecast_data).encode("utf-8")
...
class TestCandidateGeneration:
    ...
    def test_generates_single_candidate(self, collector):
        """Should generate one candidate for current forecast."""
    ...
    def test_candidate_has_correct_source_url(self, collector):
        ...
        assert candidate.source_location == "https://public-api.misoenergy.org/api/WindSolar/getwindforecast"
    ...
    def test_candidate_has_metadata(self, collector):
        ...
        assert candidate.metadata["data_type"] == "wind_forecast"
    ...
    def test_candidate_identifier_format(self, collector):
        ...
        assert candidate.identifier.startswith("wind_forecast_")
...
class TestContentCollection:
    ...
    @patch("requests.get")
    def test_successful_collection(self, mock_get, collector, sample_forecast_bytes):
        """Should fetch data from API successfully."""
        mock_response = Mock()
        mock_response.content = sample_forecast_bytes
        ...
        assert content == sample_forecast_bytes
    ...
    def test_uses_correct_headers(self, mock_get, collector):
        ...
        mock_response = Mock()
        mock_response.content = b'{"Forecast": []}'
        mock_response.raise_for_status = Mock()
        mock_get.return_value = mock_response
    ...
    def test_handles_http_error(self, mock_get, collector):
        ...
        assert "Failed to fetch wind forecast" in str(exc_info.value)
...
class TestContentValidation:
    ...
    def test_valid_forecast_data(self, collector, sample_forecast_bytes):
        """Should validate correct forecast structure."""
        candidate = collector.generate_candidates()[0]
        is_valid = collector.validate_content(sample_forecast_bytes, candidate)

        assert is_valid is True

    def test_missing_forecast_key(self, collector):
        """Should reject data without 'Forecast' key."""
        invalid_data = json.dumps({"invalid": "data"}).encode()
        candidate = collector.generate_candidates()[0]
    ...
    def test_empty_forecast_array(self, collector):
        """Should reject empty Forecast array."""
        invalid_data = json.dumps({"Forecast": []}).encode()
        candidate = collector.generate_candidates()[0]
    ...
    def test_missing_required_fields(self, collector):
        ...
        invalid_data = json.dumps({
            "Forecast": [
                {"DateTimeEST": "2025-12-02T10:00:00"}  # Missing 'Value'
            ]
        }).encode()
        candidate = collector.generate_candidates()[0]
...
class TestS3Integration:
    ...
    def test_s3_path_format(self, collector):
        ...
        candidate = DownloadCandidate(
            identifier="wind_forecast_20251202_1400.json",
            source_location="https://example.com",
            metadata={},
            collection_params={},
            file_date=date(2025, 12, 2)
        )

        s3_path = collector._build_s3_path(candidate)

        assert "s3://test-bucket/sourcing/miso_wind_forecast/" in s3_path
        assert "year=2025/month=12/day=02/" in s3_path
        assert "wind_forecast_20251202_1400.json.gz" in s3_path

    @patch("boto3.client")
    def test_upload_compresses_content(self, mock_boto_client, collector, sample_forecast_bytes):
        ...
        s3_path = "s3://test-bucket/sourcing/miso_wind_forecast/year=2025/month=12/day=02/test.json.gz"
        version_id, etag = collector._upload_to_s3(sample_forecast_bytes, s3_path)
        ...
        assert decompressed == sample_forecast_bytes
        assert len(compressed_body) < len(sample_forecast_bytes)

    @patch("boto3.client")
    def test_upload_returns_metadata(self, mock_boto_client, collector, sample_forecast_bytes):
...
class TestKafkaIntegration:
    ...
    def test_kafka_disabled_by_default(self, collector, sample_forecast_bytes):
    ...
    def test_kafka_message_structure(self, mock_kafka_config, mock_kafka_producer, collector_with_kafka):
        ...
        assert published_message.dataset == "miso_wind_forecast"
...
class TestEndToEndCollection:
    ...
    @patch("requests.get")
    @patch("boto3.client")
    def test_full_collection_run(self, mock_boto_client, mock_get, collector, sample_forecast_bytes):
    ...
    @patch("requests.get")
    def test_skips_duplicate_content(self, mock_get, collector, sample_forecast_bytes):
