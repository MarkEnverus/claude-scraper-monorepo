...
Collects Day-Ahead Ex-Post Locational Marginal Pricing data from MISO's Pricing API.
Endpoint: https://apim.misoenergy.org/pricing/v1/day-ahead/{date}/lmp-expost

Ex-Post LMP data represents final settled prices for the day-ahead energy market across
MISO's commercial pricing nodes. This data is essential for financial settlement, analysis,
and reporting. Data is typically available at 2:00 PM EST on the day before the operating day.
...
class MisoDayAheadExPostLMPCollector(BaseCollector):
    """Collector for MISO Day-Ahead Ex-Post LMP data."""
    ...
    def generate_candidates(self, **kwargs) -> List[DownloadCandidate]:
        ...
        MISO publishes Ex-Post LMP data daily, available at 2pm EST the day before
        the operating day. Each day returns paginated JSON with LMP data for all
        commercial pricing nodes (CPNodes) and all 24 hourly intervals.
        """
        candidates = []
        ...
        while current_date <= self.end_date:
            ...
            identifier = f"da_expost_lmp_{date_compact}.json"
            url = f"{self.BASE_URL}/{date_str}/lmp-expost"
            ...
            candidate = DownloadCandidate(
                ...
                collection_params={
                    "headers": {
                        ...
                        "User-Agent": "MISO-DA-ExPost-LMP-Collector/1.0",
                    },
    ...
    def collect_content(self, candidate: DownloadCandidate) -> bytes:
        ...
        The Ex-Post LMP endpoint returns paginated data with potentially hundreds of pages.
        Expected volume: ~72,000-120,000 records per day across multiple pages.
        """
        logger.info(f"Fetching DA Ex-Post LMP data from {candidate.source_location}")
        ...
        while has_more_pages:
            try:
                ...
                raise ScrapingError(f"HTTP error fetching Ex-Post LMP data: {e}") from e
            except requests.exceptions.RequestException as e:
                raise ScrapingError(f"Failed to fetch Ex-Post LMP data: {e}") from e
    ...
    def validate_content(self, content: bytes, candidate: DownloadCandidate) -> bool:
        """Validate JSON structure of Ex-Post LMP data.
...
def main(
    ...
    """Collect MISO Day-Ahead Ex-Post LMP data.

    This scraper collects Ex-Post Locational Marginal Price data from MISO's Pricing API,
    including settled LMP values and their components (MEC, MCC, MLC) for all commercial
    pricing nodes across all 24 hourly intervals.

    Data is typically available at 2:00 PM EST on the day before the operating day.
    Expected volume: ~72,000-120,000 records per day (3,000-5,000 nodes × 24 intervals).
    ...
    logger.info(
        "Starting MISO DA Ex-Post LMP collection",
        extra={
    ...
    collector = MisoDayAheadExPostLMPCollector(
        api_key=api_key,
