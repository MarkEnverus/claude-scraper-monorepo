...
Collects Day-Ahead Ex-Ante Ancillary Services Market (ASM) Clearing Prices from MISO's Pricing API.
...
Supplemental, STR, Ramp-up, Ramp-down) across different reserve zones.
...
class MisoDayAheadExAnteASMMCPCollector(BaseCollector):
    """Collector for MISO Day-Ahead Ex-Ante Ancillary Services Market Clearing Prices."""
    ...
    def generate_candidates(self, **kwargs) -> List[DownloadCandidate]:
        ...
        MISO publishes ASM MCP data daily, available at 2pm EST the day before market date.
        ...
        while current_date <= self.end_date:
            ...
            identifier = f"da_exante_asm_mcp_{date_compact}.json"
            ...
            candidate = DownloadCandidate(
                ...
                collection_params={
                    "headers": {
                        ...
                        "User-Agent": "MISO-DA-ExAnte-ASM-MCP-Collector/1.0",
                    },
    ...
    def collect_content(self, candidate: DownloadCandidate) -> bytes:
        ...
        logger.info(f"Fetching DA Ex-Ante ASM MCP data from {candidate.source_location}")
...
def main(
    ...
    """Collect MISO Day-Ahead Ex-Ante Ancillary Services Market Clearing Prices.

    This scraper collects ASM MCP data from MISO's Pricing API, including prices for
    various ancillary service products (Regulation, Spin, Supplemental, STR, Ramp-up,
    Ramp-down) across different reserve zones.
    ...
    logger.info(
        "Starting MISO DA Ex-Ante ASM MCP collection",
        extra={
    ...
    collector = MisoDayAheadExAnteASMMCPCollector(
        api_key=api_key,
        start_date=start_date,
        end_date=end_date,
        dgroup="miso_da_exante_asm_mcp",
        s3_bucket=s3_bucket,
        s3_prefix="sourcing",
